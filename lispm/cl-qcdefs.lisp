...
(DEFMACRO VAR-NAME (VAR)
  ;; NAME must be first since we use ASSQ on it.
  `(CAR ,VAR))

(DEFMACRO VAR-KIND (VAR)
  `(CADR ,VAR))

(DEFMACRO VAR-TYPE (VAR)
  `(CADDR ,VAR))

(DEFMACRO VAR-USE-COUNT (VAR)
  `(CADDDR ,VAR))

(DEFMACRO VAR-LAP-ADDRESS (VAR)
  `(CAR (CDDDDR ,VAR)))

(DEFMACRO VAR-INIT (VAR)
  `(CADR (CDDDDR ,VAR)))

(DEFMACRO VAR-EVAL (VAR)
  `(CADDR (CDDDDR ,VAR)))

(DEFMACRO VAR-MISC (VAR)
  `(CADDDR (CDDDDR ,VAR)))

(DEFMACRO VAR-DECLARATIONS (VAR)
  `(CAR (CDDDDR (CDDDDR ,VAR))))

(DEFMACRO SETF-VAR-INIT (VAR VALUE)
  `(RPLACA (CDR (CDDDDR ,VAR)) ,VALUE))

(DEFMACRO SETF-VAR-KIND (VAR VALUE)
  `(RPLACA (CDR ,VAR) ,VALUE))

(DEFMACRO SETF-VAR-LAP-ADDRESS (VAR VALUE)
  `(RPLACA (CDDDDR ,VAR) ,VALUE))
...
(DEFMACRO GOTAG-PROG-TAG (GOTAG)
  ;; Name of prog-tag described.
  `(CAR ,GOTAG))

(DEFMACRO GOTAG-LAP-TAG (GOTAG)
  ;; Name of corresponding lap-tag.
  `(CADR ,GOTAG))

(DEFMACRO GOTAG-PDL-LEVEL (GOTAG)
  ;; Pdl level to pop back to when going there.
  `(CADDR ,GOTAG))

(DEFMACRO GOTAG-PROG-NAME (GOTAG)
  ;; Name of PROG this tag is in, or NIL.
  `(CADDDR ,GOTAG))

;Mustn't use SETF either.
(DEFMACRO SETF-GOTAG-PDL-LEVEL (GOTAG LEVEL)
  `(RPLACA (CDDR ,GOTAG) ,LEVEL))

(DEFMACRO MAKE-GOTAG (&REST ARGS)
  `(LIST . ,ARGS))
...
;Mustn't use DEFSTRUCT, since must run in Maclisp!
(DEFMACRO PROGDESC-NAME (DESC)
  `(CAR ,DESC))

(DEFMACRO PROGDESC-RETTAG (DESC)
  `(CADR ,DESC))

(DEFMACRO PROGDESC-IDEST (DESC)
  `(CADDR ,DESC))

(DEFMACRO PROGDESC-M-V-TARGET (DESC)
  `(CADDDR ,DESC))

(DEFMACRO PROGDESC-PDL-LEVEL (DESC)
  `(CAR (CDDDDR ,DESC)))

(DEFMACRO PROGDESC-NBINDS (DESC)
  `(CADR (CDDDDR ,DESC)))
